...
/*********************************************************************************************************************/
#if IFX_CFG_SSW_ENABLE_PMS_INIT == 1U  // 1 启动阶段需进行电源管理系统的初始化PMS
#include "IfxPmsEvr.h"
...
#if IFX_CFG_SSW_ENABLE_PMS_INIT_CHECK == 1U  // 1 启动后会进行PMS初始化检查
 ...
 * THIS CALLOUT HOOK SHALL NOT BE IMPLEMENTED AS FUNCTION CALL!
 */
#if IFX_CFG_SSW_ENABLE_LBIST == 1U  // 执行逻辑内建自检, 即检测处理器内部逻辑电路故障, 确保逻辑完整性
 ...
 * THIS CALLOUT HOOK SHALL NOT BE IMPLEMENTED AS FUNCTION CALL!
 */
#if IFX_CFG_SSW_ENABLE_MONBIST == 1U  // 启动阶段执行监控内建自检, 检查芯片内集成的监测电路是否正常运行
 ...
 */
#if IFX_CFG_SSW_ENABLE_PLL_INIT == 1U  // 1 启动时将进行锁相环PLL的初始化, 配置系统时钟频率
#include "IfxScuCcu.h"
 ...
 */
#if IFX_CFG_SSW_ENABLE_MBIST == 1U  // 执行存储器内建自检, 监测芯片内存区域如SRAM是否有缺陷
#include "IfxMtu.h"
 ...
 */
#if IFX_CFG_SSW_ENABLE_MMIC_CHECK == 1U  // 启用对多模块接口控制器的检查, MMIC用于监控和控制芯片内部多个模块间的数据通信
 ...
 * This hook is by default defined to empty call by startup implementation.
 */
#if IFX_CFG_SSW_ENABLE_SMU == 1U  // 启动阶段将执行安全管理单元报警处理
 ...
 */
#if IFX_CFG_SSW_ENABLE_EMEM_INIT == 1U  // 启动阶段将执行扩展存储器的初始化
