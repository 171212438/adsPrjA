...
#define LCF_CSA0_SIZE 8k  // Context Save Area size for Core0
#define LCF_USTACK0_SIZE 2k  // User stack size for Core0
#define LCF_ISTACK0_SIZE 1k   // Interrupt stack size for Core0
...
#define LCF_HEAP_SIZE  4k  // Heap size for core
...
#define LCF_DEFAULT_HOST LCF_CPU0  // Set CUP0 to default(host) Core
...
#define LCF_DSPR0_START 0x70000000  // Set Data Scratch-Pad RAM start address for Core0
#define LCF_DSPR0_SIZE  240k  // Size for Core0
...
#define LCF_CSA0_OFFSET     (LCF_DSPR0_SIZE - 1k - LCF_CSA0_SIZE)  // The offset address of CSA0
...
#define LCF_INTVEC0_START 0x802FE000  // CPU0 interrupt vector table start address
#define LCF_INTVEC1_START 0x805FE000
#define LCF_INTVEC2_START 0x808FE000
#define LCF_INTVEC3_START 0x809FE000

#define LCF_TRAPVEC0_START 0x80000100   // CPU0 trap vector table start address(exception, trap)
#define LCF_TRAPVEC1_START 0x80300000
#define LCF_TRAPVEC2_START 0x80600000
#define LCF_TRAPVEC3_START 0x80900000

#define LCF_STARTPTR_CPU0 0x80000000  // CPU0 reset vector start address, automatically jumps to this address to run the startup code(_START) when power-up or reset
#define LCF_STARTPTR_CPU1 0x80300100
#define LCF_STARTPTR_CPU2 0x80600100
#define LCF_STARTPTR_CPU3 0x80900100

#define LCF_STARTPTR_NC_CPU0 0xA0000000   // CPU0 non-cacheale area start pointer address(for special access requirement) 
#define LCF_STARTPTR_NC_CPU1 0xA0300100
#define LCF_STARTPTR_NC_CPU2 0xA0600100
#define LCF_STARTPTR_NC_CPU3 0xA0900100

#define INTTAB0             (LCF_INTVEC0_START)   // Alias of CPU0 interrupt vector table start address
...
derivative tc38 
{
    core tc0
    {
        architecture = TC1V1.6.2;  // Architecture Standard
    ...
    bus sri   // SRI bus definition
    {
        mau = 8;  // Minimum addressable unit is 8 bits(1byte)
        width = 32;  // Bus bit width
	...
	// Memory definition for each core about DSRAM/PSRAM, Public Flash, LMU, and User Configuration Bank
    memory dsram3  // Data Scratch Pad Ram, Data rapidly access area, 数据快速访问区
    {
        mau = 8;
        size = 96k;
        type = ram;
    ...
    memory psram3  // Program Scratch Pad Ram, Program rapidly access area, 程序代码快速访问区
    {
        mau = 8;
        size = 64k;
        type = ram;
    ...
    memory pfls0
    {
        ...
        size = 3M;
        type = rom;
        map     cached (dest=bus:sri, dest_offset=0x80000000, size=3M);   // Cached访问区域, 用于代码执行加速
        map not_cached (dest=bus:sri, dest_offset=0xa0000000, reserved, size=3M);   // Non-Cached访问区域, 特殊区域
    }
    ...
    memory cpu0_dlmu  // 每个CPU拥有64KB本地快速RAM, 支持cached和non-cached模式
    {
        mau = 8;
        size = 64k;
        type = ram;
    ...
    memory lmuram  // 128KB公共RAM(如多核共享数据)
    {
        mau = 8;
        size = 128K;
        type = ram;
    ...
    section_setup :vtc:linear  // Program entry address, entry code executes automatically after cpu reset
    {
        start_address
        (
            symbol = "_START"  // 定义程序入口地址为_START, CPU复位后自动执行此入口代码
        );
    }
    
    section_setup :vtc:linear  // 各核独立定义用户栈, 中断栈
    {
        stack "ustack_tc0" (min_size = 1k, fixed, align = 8);   // user stack definition
        stack "istack_tc0" (min_size = 1k, fixed, align = 8);   // interrupt stack definition
    ...
    section_setup :vtc:linear
    {
        copytable  // 用于程序启动时, 将初始化数据(.data/.bss)从Flash复制到RAM中
        (
            align = 4,
            dest = linear,
            table
    ...
    section_layout :vtc:linear
    {
        ...
        group (ordered)
        {
            group trapvec_tc0 (align = 8, run_addr=LCF_TRAPVEC0_START)  // 指定固定地址用于Trap向量表, 便于快速响应异常
            {
        ...
        group (ordered)
        {
            group int_tab_tc0 (ordered)  // 指定固定地址用于中断向量表, 便于快速响应中断
            {
                #include "inttab0.lsl"
            }
        ...
        group (ordered)
        {
            group  bmh_0_orig (run_addr=mem:ucb[0x0000])  // 定义芯片启动模式, 存放于专用内存区(USB), 决定芯片启动方式
            {
    ...
    section_layout :vtc:linear
    {
        ...
        #endif
        #if LCF_DEFAULT_HOST == LCF_CPU0
        group a0 (ordered, contiguous, align = 4, attributes=rw, run_addr = mem:dsram0)  // 提供数据段高校访问模式(A0/A1相对寻址), 适合TriCore处理高校指令模式
        #endif
        {
    ...
    section_layout :vtc:linear
    {
        ...
        group
        {
            group (ordered, align = 4, run_addr=mem:pfls0)  // 明确将特定的CPU0相关常量数据段存放在专用的Flash存储区pfls0
            {
                select ".rodata.Ifx_Ssw_Tc0.*";  // 启动软件的常量数据
                select ".rodata.Cpu0_Main.*";  // 主程序中使用的常量数据
                select "(.rodata.rodata_cpu0|.rodata.rodata_cpu0.*)";  // 用户自定义CPU0专属常量数据
            }
        ...
        #endif
        #if LCF_DEFAULT_HOST == LCF_CPU0
        group (ordered, align = 4, run_addr=mem:pfls0)  // 选择Flash区域, 用于存储常量数据
        #endif
        {
            select ".rodata.farConst.cpu0.32bit";  // int型常量数据
            select ".rodata.farConst.cpu0.16bit";  // short型常量数据
            select ".rodata.farConst.cpu0.8bit";  // char或byte型常量数据
            select "(.rodata|.rodata.*)";
        }
    }
    
    /* PSRAM Code selections*/
    section_layout :vtc:linear  // 为虚拟核心vtc定义内存布局规则, 空间类型为linear, 表示采用的是32为线性地址空间
    {  // section_layout内定义程序的具体内存段(.text, .data, .bss, .rodata)如何映射到物理内存位置
        ...
        group
        {
            ...
            group
            {
                group code_psram0 (ordered, attributes=rwx, copy, run_addr=mem:psram0)  // attributes表示代码段具有读、写、执行权限; run_addr表示指定代码段实际运行时存放的物理地址
                {
                    select "(.text.cpu0_psram|.text.cpu0_psram.*)";  // 用户或工具链显式定义的必须在CPU0的PSRAM中运行的高速代码
                    select "(.text.psram_text_cpu0|.text.psram_text_cpu0.*)";  // CPU0专属的用户定义的在PSRAM运行的其他代码段
                }
    ...
    section_layout :vtc:linear
    {
        ...
        {    
            ...
            group
            {
                group (ordered, align = 4, run_addr=mem:pfls0)  // 明确将CPU0核相关的重要代码段存放在特定的Flash区域(mem:pfls0), 代码段整体4字节对齐
                {
                    select ".text.Ifx_Ssw_Tc0.*";  // CPU0启动代码
                    select ".text.Cpu0_Main.*";  // CPU0主程序代码区域
                    select ".text.CompilerTasking.Ifx_C_Init";  // Tasking编译器的C语言环境初始化代码, 负责变量初始化、数据段, 常量数据段(.rodata), 变量数据段(.data/.bss)分别存储在各自专用内存区域(如PFLASH, DSRAM)
                    select "(.text.text_cpu0|.text.text_cpu0.*)";  // 用户根据需求自定义扩展的CPU0专属代码段
                }
                group (ordered, align = 4, run_addr=mem:pfls1)
                {
        ...
        #endif
        #if LCF_DEFAULT_HOST == LCF_CPU0  // 根据默认主CPU，将.text(程序代码段)定位到对应的PFLASH区域
        group (ordered, run_addr=mem:pfls0)  //ordered表示段内元素严格按照声明顺序链接和存放, run_addr明确段运行地址
        #endif
        {
            select ".text.fast.pfls.cpu0";  // 需要高速访问的快速执行代码(如高频中断)
            select ".text.slow.pfls.cpu0";  // 执行频率较低的慢速度代码
            select ".text.5ms.pfls.cpu0";  // 5ms周期调用的任务代码
            select ".text.10ms.pfls.cpu0";  // 10ms周期调用的任务代码
            select ".text.callout.pfls.cpu0";  // 用户自定义的回调(callout)函数
            select "(.text|.text.*)";
        }
    }
}
