...
IFX_CONST IfxPmsEvr_StepDownRegulatorRegConfig IfxPmsEvr_cfgPhase1Default[] = {  // Phase1阶段要配置的寄存器
...
IFX_CONST IfxPmsEvr_StepDownRegulatorRegConfig IfxPmsEvr_cfgPhase2Default[] = {  // Phase2阶段要配置的寄存器
    {&PMS_EVRSDCOEFF0.U, (0x350873B6U), (0xFFFFFFFFU)},
    {&PMS_EVRSDCOEFF1.U, (0x02946C46U), (0xFFFFFFFFU)},
};

IFX_CONST IfxPmsEvr_StepDownRegulatorRegConfig IfxPmsEvr_cfgPhase3Default[] = {  // Phase3阶段要配置的寄存器
...
IFX_CONST IfxPmsEvr_initSequence IfxPmsEvr_cfgSequenceDefault = {
    sizeof(IfxPmsEvr_cfgPhasesDefault) / sizeof(IfxPmsEvr_initSequencePhase),  // 初始化过程中的阶段数量, 3个阶段
...
{  // 配置EVR次级电压监控ADC测量值的滤波方式(单次或连续多次测量的平均值), 以降低测量误报率
...
{  // 获取特定供电模式(如EVRC, EVR33, VDDM等)的次级ADC监测最近一次的测量结果
    uint8 adcResult = 0;
...
{  // 设定EVR次级监测的过压阈值, 当对应供电模块的电压超过设定的阈值时, 触发过压事件或保护动作
    uint16 passwd = IfxScuWdt_getSafetyWatchdogPassword();
