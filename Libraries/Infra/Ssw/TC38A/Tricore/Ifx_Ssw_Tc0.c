...
#define IFX_SSW_INIT_CONTEXT() \
    { \
        Ifx_Ssw_setAddressReg(a10, __USTACK(0));  /* 加载用户指针 */ \
        Ifx_Ssw_DSYNC();  /* 数据同步 */ \
        Ifx_Ssw_initCSA((unsigned int *)__CSA(0), (unsigned int *)__CSA_END(0));  /* 初始化CPU上下文保存区域 */ \
        Ifx_Ssw_ISYNC();  /* 指令同步 */ \
    }
/*******************************************************************************
...
static void __StartUpSoftware(void)
{
    ...
    Ifx_Ssw_MTCR(CPU_PSW, IFX_CFG_SSW_PSW_DEFAULT);  // Program Status Word
