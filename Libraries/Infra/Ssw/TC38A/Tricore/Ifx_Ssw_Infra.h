...
IFX_SSW_INLINE unsigned char Ifx_Ssw_isApplicationReset(void)
{
    ...
    unsigned int  tempValue = SCU_RSTSTAT.U;  // Reset Status Resgiter

    if ((tempValue & IFX_SSW_POWERONRESET_MASK) > 0U)  // PowerOn Reset Mask
    {
        returnVal = 0U;
    }
    else if ((tempValue & IFX_SSW_APPLICATIONRESET_MASK) > 0U)  // Application Reset Mask
    {
        ...
        tempValue = (SCU_RSTCON.U >> ((31U - Ifx_Ssw_CLZ(tempValue)) << 1U)) & 3U;  // Reset Configuration Register
